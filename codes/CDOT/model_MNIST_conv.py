...
## Tune discriminator
## Labels in OT
## Time kernel / non-linear
from torch import nn
import torch
device = "cpu"

class ClassifyNet(nn.Module):
    def __init__(self,data_shape, hidden_shape, n_classes, append_time=False,encode_time=False,time_conditioning=False,leaky=True):
        ...
        self.relu_0    = nn.ReLU() #TimeReLU(1352*in_channels,2,leaky)
        # self.relu_0 = nn.ReLU()
        if time_conditioning:
            ...
            self.relu_1 = nn.ReLU() #TimeReLU(200,2,leaky)
            self.relu_2 = nn.ReLU() #TimeReLU(72,2,leaky)
            # self.relu_t = nn.ReLU()
        self.down_sampling = nn.MaxPool2d(kernel_size=2, stride=2)
        self.out_layer = nn.Linear(1352,n_classes)
        self.out_relu = nn.ReLU() #TimeReLU(n_classes,2,leaky)
