...
class TransformerTrainer():
    def __init__(self,args):
        ...
        self.source_data_indices = json.load(open(data_index_file,"r"))
            ...
            for j in range(i,len(self.source_domain_indices)):
                if i!=j:
                    ...
                    if self.encoder is not None:
                        Xs = self.encoder(next(iter(ot_data[i]))[0]).view(len(self.source_domain_indices[i]),-1).detach().cpu().numpy()+1e-6
                        ys = next(iter(ot_data[i]))[3].view(len(self.source_data_indices[i]),-1).detach().cpu().numpy()
                        Xt = encoder(next(iter(ot_data[j]))[0]).view(len(self.source_data_indices[j]),-1).detach().cpu().numpy()+1e-6
                        yt = next(iter(ot_data[j]))[3].view(len(self.source_data_indices[j]),-1).detach().cpu().numpy()
                    else:
                        Xs = next(iter(ot_data[i]))[0].view(len(self.source_data_indices[i]),-1).detach().cpu().numpy()+1e-6
                        ys = next(iter(ot_data[i]))[3].view(len(self.source_data_indices[i]),-1).detach().cpu().numpy()
                        Xt = next(iter(ot_data[j]))[0].view(len(self.source_data_indices[j]),-1).detach().cpu().numpy()+1e-6
                        yt = next(iter(ot_data[j]))[3].view(len(self.source_data_indices[j]),-1).detach().cpu().numpy() 
                    ot_sinkhorn.fit(Xs=Xs, ys=ys, Xt=Xt, yt=yt, iteration=0)
                    self.ot_maps[i][j] = ot_sinkhorn.transform(Xs).reshape(self.out_shape)
                else:
                    if  self.encoder is not None:
                        Xs = self.encoder(next(iter(ot_data[i]))[0])
                    else:
    ...
    def train_transformer(self):
        ...
        for index in range(1, len(self.source_domain_indices)):
            ...
            print('----------------------------------------------------------------------------------------------')
            past_dataset = torch.utils.data.DataLoader(self.DataSet(indices=self.cumulative_data_indices[index-1],**self.dataset_kwargs),self.BATCH_SIZE,True)  # Used to train discriminator 

            curr_dataset = torch.utils.data.DataLoader(self.DataSet(indices=self.source_data_indices[index],**self.dataset_kwargs),self.BATCH_SIZE,True,drop_last=True)  # Used to train discriminator

            num_past_batches = len(self.cumulative_data_indices[index-1]) // self.BATCH_SIZE
            all_dataset = torch.utils.data.DataLoader(self.DataSet(indices=self.cumulative_data_indices[index],transported_samples=self.ot_maps,target_bin=index,**self.dataset_kwargs),self.BATCH_SIZE,True)   # Used to train transformer
    ...
    def train_final_classifier(self):
        # for i in range(len(self.target_domain_indices)):
            # We train only on 1 final target_domain !
            # print(U_target[i])
