...
class ClassifyNetHuge(nn.Module):
    ...
    def __init__(self,input_shape=32,hidden_shapes=[400,400],output_shape=1, **kwargs):
        ...
        if (kwargs['time_softmax'] if kwargs.get('time_softmax') else False):
            self.time_softmax = nn.Sequential(nn.Linear(self.time_shape,50),
                                               nn.LeakyReLU(),
                                               nn.Linear(50,2*self.output_shape))
        else:
    ...
    def forward(self,X,times=None,logits=False,delta=0.0):
        ...
        if self.time_softmax is not None:
            # print(times)
            time_softmax = self.time_softmax(times.view(-1,self.time_shape))
            # print("-----------------",time_softmax.size()) 
            # print(time_softmax[:,:self.output_shape])
            X = X*time_softmax[:,:self.output_shape] + time_softmax[:,self.output_shape:]
        ...
        if self.regress:
            if X.size(1) == 2:
                X_ = torch.relu(X[:,0])
                X__ = X[:,1]**2
                X = torch.cat([X_.view(-1,1),X__.view(-1,1)],dim=1)
            else:
                X = torch.relu(X)
            # X[:,1:] = X[:,1:]**2
            # pass
