...
class RegularizedSinkhornTransport(BaseTransport):
    ...
    def __init__(self, reg_e=0.1, alpha=0.1, max_iter=1000, tol=10e-9, verbose=False, log=False,
    ...
    def fit(self, Xs=None, ys=None, Xt=None, yt=None, prev_gamma=None, iteration=1):
        ...
        super(RegularizedSinkhornTransport, self).fit(Xs=Xs, ys=None, Xt=Xt, yt=None)
        ...
        for k in range(5):
            ...
            if self.log:
                gamma_t, log = sinkhorn_stabilized(a=self.mu_s, b=self.mu_t, M=cost_k, reg=1+self.reg_e*self.alpha,
                                     numItermax=self.max_iter, stopThr=self.tol, verbose=self.verbose, log=self.log)

            else:
                gamma_t = sinkhorn_stabilized(a=self.mu_s, b=self.mu_t, M=cost_k, reg=1+self.reg_e*self.alpha,
                                 numItermax=self.max_iter, stopThr=self.tol, verbose=self.verbose, log=self.log)

            gamma_t = gamma_t / (np.sum(gamma_t, 1)[:, None] + 1e-10)
