...
class RotMNIST(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self,indices,bin_width,bin_index,n_bins,transported_samples=None,target_bin=None,n_samples=6000,vgg=False,verbose=False):
        '''
        ...
        self.vgg = vgg
        self.means = torch.tensor([0.485, 0.456, 0.406]).view(3,1,1).to(device)
        self.stds  = torch.tensor([0.229, 0.224, 0.225]).view(3,1,1).to(device)
    ...
    def __getitem__(self,idx):
        ...
        return image.float().to(device),torch.cat([(bin/self.n_bins).float().view(1),torch.tensor(norm_angle).float().to(device).view(1)],dim=0).to(device),target
...
class RotMNISTCGrad(torch.utils.data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        return image.to(device),target_image.to(device), torch.tensor(norm_angle - target_norm_angle).to(device)
    def __len__(self):
