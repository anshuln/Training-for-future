...
class RotMNIST(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self,indices,bin_width,bin_index,n_bins,n_samples=60000):
        '''
        ...
        self.angles  = self.bins*bin_width + np.random.randint(bin_width)
    ...
    def __getitem__(self,idx):
        ...
        bin   = torch.tensor(self.bins[idx]).to(device)
        angle = self.angles[idx]
        norm_angle = self.normalized_angles[idx]
        image = self.data[index]
        image = Image.fromarray(image.numpy(), mode='L')

        target = self.targets[index]
        image = np.array(rotate(image,angle))#).float().to(device)
        image = torch.tensor(image).to(torch.float).to(device)
        target = torch.tensor(target).to(device)
        # print(bin,norm_angle)
        return image,torch.cat([bin.view(1),torch.tensor(norm_angle).float().to(device).view(1)],dim=0),target
