...
class ClassifyNet(nn.Module):
    def __init__(self,data_shape, hidden_shape, n_classes, time_conditioning=True,leaky=True):
...
class TimeEncodings(nn.Module):
    ...
    def forward(self,X,times):
        odd_indices = torch.arange(0,times.size(1),2).long().to(device)
        even_indices = torch.arange(1,times.size(1),2).long().to(device)
        times = torch.cat([times,times.gather(1,odd_indices)-times.gather(1,even_indices)],dim=1)
        if self.proj_time:
        ...
        for t in range(times.size(1)):
            freq = torch.tensor([1/((100/(t+1))**(2*x/self.model_dim)) for x in range(self.model_dim)]).view(1,self.model_dim).repeat(n,1).to(device)
            offsets = torch.ones_like(X).to(device) * (3.1415/2) * (t % 2)
            pos = torch.sin(freq * times[:,t] + offsets)
            X = X + pos
